...
SQL minta:
```bash
sqlite3 {{INSTALL_DIR}}/store/noa.db "SELECT agent_id, content, keywords FROM memories WHERE created_at > strftime('%s', 'now', '-24 hours') AND category IN ('hot','warm') ORDER BY agent_id, created_at"
```
...
sqlite3 {{INSTALL_DIR}}/store/noa.db "SELECT COUNT(*) as total, COUNT(embedding) as with_emb FROM memories"
...
sqlite3 {{INSTALL_DIR}}/store/noa.db "SELECT id, content, accessed_at FROM memories WHERE category='hot' AND accessed_at < strftime('%s', 'now', '-7 days')"
```
...
```bash
sqlite3 {{INSTALL_DIR}}/store/noa.db "UPDATE memories SET category='cold' WHERE id IN (...)"
```
...
sqlite3 {{INSTALL_DIR}}/store/noa.db "SELECT id, title, status, project, priority, assignee FROM kanban_cards WHERE status IN ('planned','in_progress','waiting') AND archived_at IS NULL ORDER BY project, priority DESC"
```
