#!/bin/bash
# Watchdog for agent-thor (QA/tester agent, per-agent Telegram channel).
#
# Unlike dave-watchdog (which relaunches with --continue for task continuity),
# Thor is a CHANNEL agent: a --continue relaunch loses the --channels activation
# state ("server not in --channels list") and can hit the "No deferred tool
# marker" immediate-exit. So Thor is ALWAYS relaunched FRESH with --channels +
# its own TELEGRAM_STATE_DIR. Thor's durable state lives in the memory system,
# not the session transcript, so a fresh session is safe.
#
# Model is read from agent-config.json on every (re)launch.

SESSION=agent-thor
AGENT_DIR=/home/domin/marveen/agents/thor
CFG="$AGENT_DIR/.claude-config"
STATE="$AGENT_DIR/.claude/channels/telegram"
ACONF="$AGENT_DIR/agent-config.json"
LOG=/home/domin/marveen/store/thor-watchdog.log
COOLDOWN=60
MAX_PER_HOUR=8
