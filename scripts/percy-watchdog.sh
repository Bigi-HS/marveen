#!/bin/bash
# Watchdog for agent-percy (Dominik's personal D&D master / character assistant,
# per-agent Telegram channel).
#
# Unlike dave-watchdog (which relaunches with --continue for task continuity),
# Percy is a CHANNEL agent: a --continue relaunch loses the --channels activation
# state ("server not in --channels list") and can hit the "No deferred tool
# marker" immediate-exit. So Percy is ALWAYS relaunched FRESH with --channels +
# its own TELEGRAM_STATE_DIR. Percy's durable state lives in the memory system
# and the D&D KB (store/dnd-kb/), not the session transcript, so a fresh session
# is safe.
#
# Model is read from agent-config.json on every (re)launch.

SESSION=agent-percy
AGENT_DIR=/home/domin/marveen/agents/percy
CFG="$AGENT_DIR/.claude-config"
STATE="$AGENT_DIR/.claude/channels/telegram"
ACONF="$AGENT_DIR/agent-config.json"
LOG=/home/domin/marveen/store/percy-watchdog.log
COOLDOWN=60
MAX_PER_HOUR=8
...
log "percy-watchdog started (pid $$)"
