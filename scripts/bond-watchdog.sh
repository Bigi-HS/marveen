#!/bin/bash
# Watchdog for agent-bond (English-tutor agent, per-agent Telegram channel).
#
# Bond is a CHANNEL agent (own bot @Bond_Tutor_bot): a --continue relaunch loses
# the --channels activation state ("server not in --channels list") and can hit
# the "No deferred tool marker" immediate-exit. So Bond is ALWAYS relaunched
# FRESH with --channels + its own TELEGRAM_STATE_DIR. Bond's durable state lives
# in bond.db + the memory system, not the session transcript, so a fresh session
# is safe (mirrors thor-watchdog).
#
# Model is read from agent-config.json on every (re)launch.

SESSION=agent-bond
AGENT_DIR=/home/domin/marveen/agents/bond
CFG="$AGENT_DIR/.claude-config"
STATE="$AGENT_DIR/.claude/channels/telegram"
ACONF="$AGENT_DIR/agent-config.json"
LOG=/home/domin/marveen/store/bond-watchdog.log
COOLDOWN=60
MAX_PER_HOUR=8
