...
def _require_committed(source, repo_root):
    """The source must be tracked AND have no uncommitted changes (tracked + clean).

    IMPORTANT: this check does NOT verify that the source has been reviewed or
    merged to develop.  A committed-but-unreviewed edit passes this gate.  The
    real review gate -- blob presence on origin/develop -- is a separate check
    (see card 8c754df8 Part 2).  Until that lands, this function is a
    necessary-but-not-sufficient pre-condition for promotion."""
