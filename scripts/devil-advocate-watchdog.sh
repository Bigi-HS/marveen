#!/bin/bash
# Watchdog for agent-devil-advocate (adversarial red-team agent, per-agent
# Telegram channel as of 2026-06-11).
#
# Like Thor/Chad, this is a CHANNEL agent: a --continue relaunch loses the
# --channels activation state, so it is ALWAYS relaunched FRESH with --channels
# + its own TELEGRAM_STATE_DIR. Durable state lives in the memory system, not
# the session transcript, so a fresh session is safe.
#
# Model is read from agent-config.json on every (re)launch.

SESSION=agent-devil-advocate
AGENT_DIR=/home/domin/marveen/agents/devil-advocate
CFG="$AGENT_DIR/.claude-config"
STATE="$AGENT_DIR/.claude/channels/telegram"
ACONF="$AGENT_DIR/agent-config.json"
LOG=/home/domin/marveen/store/devil-advocate-watchdog.log
COOLDOWN=60
MAX_PER_HOUR=8
...
log "devil-advocate-watchdog started (pid $$)"
