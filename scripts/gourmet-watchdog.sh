#!/bin/bash
# Watchdog for agent-gourmet (recipe/OCR agent, per-agent Telegram channel).
#
...
SESSION=agent-gourmet
AGENT_DIR=/home/domin/marveen/agents/gourmet
CFG="$AGENT_DIR/.claude-config"
STATE="$AGENT_DIR/.claude/channels/telegram"
ACONF="$AGENT_DIR/agent-config.json"
LOG=/home/domin/marveen/store/gourmet-watchdog.log
COOLDOWN=60
MAX_PER_HOUR=8
...
log "gourmet-watchdog started (pid $$)"
