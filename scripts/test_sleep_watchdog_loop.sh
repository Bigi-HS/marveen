...
. "$ROOT/scripts/lib/pane-idle.sh"      # REAL debounce accrual (pane_idle_accrue)
...
BUSY_FILE="$STATE_DIR/$NAME.busy-count"
PIN="$TMP/pin-$NAME"
DB="$TMP/noa.db"
POLL=15
...
reset_db() {
  rm -f "$DB"
  sgt_sql "$DB" "CREATE TABLE agent_messages(to_agent TEXT,status TEXT,delivered_at INTEGER,completed_at INTEGER,created_at INTEGER,ack_expected INTEGER DEFAULT 0);
...
MOCK_PANE_STATE=idle  # pane_capture_classify verdict: idle | working | ambiguous
...
pane_capture_classify() { printf '%s' "$MOCK_PANE_STATE"; }   # stub: no live tmux capture
...
reset_db; MOCK_SESSION=0; rm -f "$MARKER" "$IDLE_FILE" "$BUSY_FILE"
...
reset_db; sgt_sql "$DB" "INSERT INTO agent_messages VALUES('$NAME','pending',NULL,NULL,$MOCK_NOW,0);"
MOCK_SESSION=0; rm -f "$MARKER" "$IDLE_FILE" "$BUSY_FILE"
...
reset_db; sgt_sql "$DB" "INSERT INTO agent_messages VALUES('$NAME','pending',NULL,NULL,$MOCK_NOW,0);"
MOCK_SESSION=0; MOCK_UNDER_CAP=1; rm -f "$MARKER" "$IDLE_FILE" "$BUSY_FILE"
...
reset_db; MOCK_SESSION=1; MOCK_PANE_STATE=working; rm -f "$IDLE_FILE" "$BUSY_FILE" "$MARKER"
...
reset_db; MOCK_SESSION=1; MOCK_PANE_STATE=idle; rm -f "$IDLE_FILE" "$BUSY_FILE" "$MARKER"
...
reset_db; MOCK_SESSION=1; MOCK_PANE_STATE=idle; rm -f "$MARKER"
...
# Post-FINDING-1 (#691) "open obligation" = undelivered OR delivered-with-ack_expected=1.
# Use a delivered message the agent must still act on (ack_expected=1).
reset_db; sgt_sql "$DB" "INSERT INTO agent_messages VALUES('$NAME','delivered',$MOCK_NOW,NULL,$MOCK_NOW,1);"
MOCK_SESSION=1; MOCK_PANE_STATE=idle; rm -f "$MARKER" "$BUSY_FILE"
echo "$((MOCK_NOW-2000))" > "$IDLE_FILE"
step; assert_actions "ADV-2 awake + idle + open obligation (ack_expected=1) -> NO kill (R4)" ""

# ADV-2b (FINDING-1 behavior lock): a DELIVERED FYI with ack_expected=0 is NOT an
# open obligation -> a long-idle agent with only a delivered FYI DOES sleep. This
# is the exact predicate narrowing #691 shipped; assert it at the loop level too.
reset_db; sgt_sql "$DB" "INSERT INTO agent_messages VALUES('$NAME','delivered',$MOCK_NOW,NULL,$MOCK_NOW,0);"
MOCK_SESSION=1; MOCK_PANE_STATE=idle; rm -f "$MARKER" "$BUSY_FILE"
echo "$((MOCK_NOW-2000))" > "$IDLE_FILE"
step; assert_actions "ADV-2b awake + idle + delivered FYI (ack_expected=0) -> kill (FINDING-1)" "KILL"

# S10: awake + idle long enough but in_progress card -> NO kill (5.2.2).
reset_db; sgt_sql "$DB" "INSERT INTO kanban_cards VALUES('$NAME','in_progress');"
MOCK_SESSION=1; MOCK_PANE_STATE=idle; rm -f "$MARKER"
echo "$((MOCK_NOW-2000))" > "$IDLE_FILE"
...
reset_db; MOCK_SESSION=1; MOCK_PANE_STATE=working; rm -f "$IDLE_FILE"
...
# ===========================================================================
# FINDING-2 debounce scenarios (card 089e78db PART 2): a single ambiguous poll
# (transient redraw frame) must NOT sleep the agent NOR zero an accumulated idle
# timer; only sustained ambiguity resets it. Uses the REAL pane_idle_accrue.
# ===========================================================================
export PANE_IDLE_DEBOUNCE_POLLS=3

# D1: awake + one ambiguous poll on an almost-ripe timer -> NO kill, timer kept.
reset_db; MOCK_SESSION=1; MOCK_PANE_STATE=ambiguous; rm -f "$MARKER" "$BUSY_FILE"
echo "$((MOCK_NOW-1799))" > "$IDLE_FILE"   # 1s short of the 1800s threshold
step; assert_actions "D1 awake + 1 ambiguous poll -> NO kill (R5, never mid-input)" ""
[ -f "$IDLE_FILE" ] && ok "D1 idle timer PRESERVED across a transient ambiguous frame" || bad "D1 idle file must survive a single ambiguous poll"

# D2: sustained ambiguity (reach k=3 consecutive) -> idle timer reset (real activity).
reset_db; MOCK_SESSION=1; MOCK_PANE_STATE=ambiguous; rm -f "$MARKER" "$BUSY_FILE"
echo "$((MOCK_NOW-2000))" > "$IDLE_FILE"   # already past threshold
step; assert_actions "D2a ambiguous poll #1 -> NO kill" ""
step; assert_actions "D2b ambiguous poll #2 -> NO kill" ""
[ -f "$IDLE_FILE" ] && ok "D2 idle timer still held at poll #2 (<k)" || bad "D2 idle file must survive below-threshold ambiguity"
step; assert_actions "D2c ambiguous poll #3 (>=k) -> NO kill" ""
[ ! -f "$IDLE_FILE" ] && ok "D2 idle timer RESET after k sustained ambiguous polls" || bad "D2 idle file must reset at k consecutive ambiguous"

# D3: an idle poll after a transient ambiguous blip resumes the OLD timer and can sleep.
reset_db; MOCK_SESSION=1; rm -f "$MARKER" "$BUSY_FILE"
echo "$((MOCK_NOW-2000))" > "$IDLE_FILE"   # ripe
MOCK_PANE_STATE=ambiguous; step; assert_actions "D3a one ambiguous blip -> NO kill" ""
MOCK_PANE_STATE=idle;      step; assert_actions "D3b idle resumes ripe timer -> kill (sleep)" "KILL"
