...
run_once() {
  ...
  RESPAWN_CMD="export PATH=\"/opt/homebrew/bin:\$HOME/.bun/bin:/home/linuxbrew/.linuxbrew/bin:\$HOME/.local/bin:/usr/local/bin:/usr/bin:/bin\" && export FLEET_ROOT=\"$INSTALL_DIR\" && . \"$INSTALL_DIR/scripts/lib/fleet-oauth-env.sh\" && $CLAUDE --dangerously-skip-permissions --effort max ${MODEL_FLAG}--channels plugin:telegram@claude-plugins-official"
