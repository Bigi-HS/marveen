#!/bin/bash
# Watchdog for agent-hibiki (personal-trainer agent, per-agent Telegram channel).
#
# Unlike dave-watchdog (which relaunches with --continue for task continuity),
# Hibiki is a CHANNEL agent: a --continue relaunch loses the --channels activation
# state ("server not in --channels list") and can hit the "No deferred tool
# marker" immediate-exit. So Hibiki is ALWAYS relaunched FRESH with --channels +
# its own TELEGRAM_STATE_DIR. Hibiki's durable state lives in the memory system,
# not the session transcript, so a fresh session is safe.
#
# Model is read from agent-config.json on every (re)launch.

SESSION=agent-hibiki
AGENT_DIR=/home/domin/marveen/agents/hibiki
CFG="$AGENT_DIR/.claude-config"
STATE="$AGENT_DIR/.claude/channels/telegram"
ACONF="$AGENT_DIR/agent-config.json"
LOG=/home/domin/marveen/store/hibiki-watchdog.log
COOLDOWN=60
MAX_PER_HOUR=8
...
log "hibiki-watchdog started (pid $$)"
