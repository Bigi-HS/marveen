    ...
    for action in ("activate", "deactivate"):
        try:
            req = urllib.request.Request(
                f"{N8N_BASE}/workflows/{wf_id}/{action}",
                data=b"{}",
                headers=headers,
                method="POST",
            )
            with urllib.request.urlopen(req) as r:
                if r.status not in (200, 201):
                    print(f"[ghost-flush] {wf_id} {action}: unexpected HTTP {r.status}")
                    return False
        except Exception as e:
            print(f"[ghost-flush] {wf_id} {action} FAILED: {e}")
            return False
    return True
...
def main() -> int:
    ...
    boot_iso = datetime.fromtimestamp(boot, timezone.utc).isoformat()
