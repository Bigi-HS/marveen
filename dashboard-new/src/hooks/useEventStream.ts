...
export type DashboardEventType = 'kanban' | 'message' | 'board' | 'gate' | 'agent-status' | 'hook-event'

const STREAM_PATH = '/api/events/stream'
const EVENT_TYPES: DashboardEventType[] = ['kanban', 'message', 'board', 'gate', 'agent-status', 'hook-event']
...
function zeroRevisions(): Revisions {
  return { kanban: 0, message: 0, board: 0, gate: 0, 'agent-status': 0, 'hook-event': 0 }
}
