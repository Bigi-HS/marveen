...
/** Human label for the status dot (tooltip / a11y). Hungarian: the UI reads in
 *  Boss's language (the gate/hook pages already do). */
 ...
 * borrows the hottest palette colour, orange, but its "Beragadt" label keeps it
 * distinct from the (also-orange, pulsing) busy state. Idle is deliberately QUIET
 * (muted, no cyan): with most of the fleet idle a cyan pill on every card floods
 * the grid and cyan loses its "live/data" meaning (DESIGN.md). The cyan status
 * dot still marks idle agents as alive; the pill stays visual rest.
 */
export const AGENT_STATUS_CHIP: Record<AgentLiveStatus, string> = {
  idle: 'text-text-muted bg-bg-elevated/60 ring-1 ring-inset ring-border',
 ...
 * palette colour); high as outlined orange; normal and low muted. Flagged to
 * Quill as a minor spec note (no red token in store/dashboard-palette-noa.md).
 * normal/low use text-muted, not text-neutral: neutral on bg-surface is ~4.3:1,
 * below the 4.5:1 WCAG AA floor at this 10px size (text-muted is ~6:1).
 */
export const PRIORITY_BADGE: Record<Priority, string> = {
  urgent: 'bg-primary text-bg-deep font-semibold',
  high: 'border border-primary text-primary',
  normal: 'border border-border text-text-muted',
  low: 'border border-border/60 text-text-muted',
}
