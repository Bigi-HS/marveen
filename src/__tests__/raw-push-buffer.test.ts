...
describe('RawPushBuffer', () => {
  ...
  // SEC (Chad #584): the buffer holds health PII -- the file must be owner-only (0o600).
  it('SEC: creates the retention file with 0o600 permissions', () => {
    const { buf: b, dir: d } = makeTmpBuffer()
    b.retain('{"vitals":{"resting_hr_bpm":52}}')
    const mode = statSync(join(d, 'test-buffer.jsonl')).mode & 0o777
    expect(mode).toBe(0o600)
    rmSync(d, { recursive: true, force: true })
  })

  it('SEC: re-tightens permissions to 0o600 even if the file pre-existed at 0o644', () => {
    const { buf: b, dir: d } = makeTmpBuffer()
    const fp = join(d, 'test-buffer.jsonl')
    // Simulate a buffer file written by pre-guard code at a looser mode.
    writeFileSync(fp, '"legacy"\n', { encoding: 'utf8', mode: 0o644 })
    chmodSync(fp, 0o644)
    b.retain('{"push":1}')
