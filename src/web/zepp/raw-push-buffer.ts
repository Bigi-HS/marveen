...
export class RawPushBuffer {
  ...
  retain(rawBody: string): void {
    ...
    try {
      // mode 0o600: the buffer holds health PII (sleep, vitals, steps); keep it owner-only.
      // writeFileSync's mode applies only on creation, so chmod after to enforce 0o600 even
      // if the file pre-existed at a looser mode (e.g. a buffer written before this guard).
