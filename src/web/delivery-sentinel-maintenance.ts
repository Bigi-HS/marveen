// Boot-time maintenance for the delivery sentinel JSONL trails (card 681f99b0 /
// A2). Both trails under store/ are append-only and were never bounded:
...
//     newest rows.
//
// Run ONCE at server boot, before the ACK clear-observer starts, so the observer
// reads the freshly-compacted file. The pure decisions live in delivery-ack.ts /
// delivery-alert.ts and are unit-tested; this module is the IO seam. Every step
// is wrapped so a maintenance failure never blocks startup. Writes are atomic
// (tmp + rename) so a crash mid-rewrite cannot leave a torn trail.
