...
export class ScopedSharedError extends Error {
  // Card MEM/8ad3a1c9: the old message named `access_scope` unconditionally, which
  // sent the reader looking at a field they had never sent -- Hibiki bisected the
  // CONTENT down to a single character before finding the real cause. When the
  // scope was derived from the content, say so.
  constructor(derivedFromContent = false) {
    super(derivedFromContent
      ? 'shared memory rejected: the content matched the PII classifier, so it was auto-assigned an access_scope, and a shared memory cannot be scope-restricted. You did not send access_scope -- it came from the text. Rephrase, or send category other than shared.'
      : 'scoped+shared is contradictory: a shared memory cannot also be access_scope-restricted')
...
// Card MEM/8ad3a1c9: these matched as BARE SUBSTRINGS, so any word merely
// containing the letters counted. "listaja" (= its list) contains t-a-j and was
// classified as health data; "gzip" hit 'zip', "hibanak" hit 'iban', "specimen"
// hit 'cim'. The match now has to start at a word boundary.
//
// A start boundary only, NOT \b...\b on both sides. Hungarian glues suffixes onto
// the stem, so "tajszamom", "lakcimem", "egeszsegadat" and "diagnozist" are all
// genuine hits that a two-sided boundary would have silently dropped -- that would
// be a weakening of a filter that exists to protect the health agent, dressed up
// as a false-positive fix. Measured on 808k chars of Hungarian fleet prose: the
// start boundary removes 45 of the 47 phantom 'taj' matches and keeps every
// agglutinated form.
const PII_PATTERNS = PII_KEYWORDS.map(
  (kw) => new RegExp(`(?:^|[^\\p{L}\\p{N}])${deaccent(kw).replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`, 'u'),
)

export function isPotentialPII(keywords: string | null | undefined, content: string): boolean {
  const hay = deaccent(`${keywords ?? ''}\n${content}`)
  return PII_PATTERNS.some((re) => re.test(hay))
}
...
export function resolveAccessScope(
  ...
  let derivedFromContent = false
  if (accessScope === undefined) {
    derivedFromContent = isPotentialPII(keywords, content)
    effective = derivedFromContent ? agentId : null
  } else if (accessScope === null || accessScope === '') {
    effective = null
  } else {
    effective = accessScope
  }
  if (category === 'shared' && effective !== null) throw new ScopedSharedError(derivedFromContent)
  return effective
}
...
export function saveMemory(
...
): { id: number; access_scope: string | null } {
  ...
  // Card MEM/8ad3a1c9: report the scope that was WRITTEN. Content-derived scoping
  // used to happen with no trace in the response at all, so a caller could not
  // tell an unscoped write from one the classifier had quietly restricted.
  return { id, access_scope: scope }
}
